...
const FilterBtn: React.FC<TFilterBtnProps & React.HTMLAttributes<HTMLDivElement>> = ({
  ...
  return (
    <div
      {...props}
      className={cls(
        'inline-flex items-center bg-secondary cursor-pointer p-8 rounded-7 border tablet:p-11 tablet:rounded-9 desktop:p-12 desktop:rounded-10',
    ...
    >
      ...
      <ArrowDown
        className={cls(
          'w-17 h-17 stroke-2 tablet:w-22 tablet:h-22 desktop:w-24 desktop:h-24',
          isOn ? 'stroke-main' : 'stroke-primary',
        )}
      />
    </div>
  );
